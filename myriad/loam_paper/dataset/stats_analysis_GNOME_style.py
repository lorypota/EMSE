...
def compare_proportions(df):
    ...
    return results
def compute_specialization(contrib_df, task_cols, method="entropy"):
    """
    Compute one specialization score per contributor-repo pair
    based on task proportions.

    method:
      - "entropy": low = specialized, high = diverse
      - "gini": high = specialized, low = diverse
    """
    prop_cols = [f"{t}_prop" for t in task_cols]
    P = contrib_df[prop_cols].values

    eps = 1e-12  # avoid log(0)

    if method == "entropy":
        # Shannon entropy
        contrib_df["specialization"] = -(P * np.log(P + eps)).sum(axis=1)

    elif method == "gini":
        # Gini over proportions
        def gini(x):
            x = np.sort(x)
            n = len(x)
            if x.sum() == 0:
        ...
        contrib_df["specialization"] = [gini(row) for row in P]
...
def compare_specialization(df, method="entropy"):
    print("\n" + "="*70)
    print(f"SPECIALIZATION COMPARISON ({method.upper()}): CORE VS PERIPHERAL")
    print("="*70)

    contrib_df, task_cols = compute_proportions(df)
    contrib_df = compute_specialization(contrib_df, task_cols, method=method)

    core_vals = contrib_df[contrib_df["core_status"] == "core"]["specialization"].values
    periph_vals = contrib_df[contrib_df["core_status"] == "peripheral"]["specialization"].values
    ...
    print(f"Core median:      {np.median(core_vals):.4f}")
    print(f"Peripheral median:{np.median(periph_vals):.4f}")
...
def generate_specialization_violin(df, output_path, method="entropy"):
    """
    Violin plot of specialization scores (entropy or gini) for:
    - Core contributor–repo pairs
    - Peripheral contributor–repo pairs
    """
    print("\n" + "="*70)
    print("GENERATING SPECIALIZATION VIOLIN PLOT")
    print("="*70)
    ...
    # Split
    core_vals = contrib_df[contrib_df["core_status"] == "core"]["specialization"].dropna().values
    periph_vals = contrib_df[contrib_df["core_status"] == "peripheral"]["specialization"].dropna().values

    data = [core_vals, periph_vals]
    labels = [f"Core (n={len(core_vals)})", f"Peripheral (n={len(periph_vals)})"]
    ...
    ax.set_ylabel(f"Specialization score ({method})", fontsize=12)
    ax.set_xlabel("Contributor role", fontsize=12)

    # Useful reference line for entropy max
    if method == "entropy":
        max_entropy = np.log(len(task_cols))
        ax.axhline(max_entropy, linestyle="--", linewidth=1)
        ax.text(1.02, max_entropy, f"max = ln({len(task_cols)}) = {max_entropy:.3f}", va="bottom", fontsize=9)
    ...
    print(f"Saved specialization violin plot to {output_path}")
...
def main():
    ...
    parser.add_argument('--csv', required=True, help='Path to contributors_tasks_GNOME_based.csv')
    parser.add_argument('--spec-figure', default='rq2_specialization_violin.png', help='Output path for specialization violin plot')
    ...
    generate_specialization_violin(df, args.spec_figure, method="entropy")
    compare_specialization(df, method="entropy") 
