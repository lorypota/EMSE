...
Output: contributors_tasks_GNOME_based.csv with columns:
...
def process_fetched_commits(fetched_dir, repo_name):
    ...
    contributor_data = defaultdict(lambda: defaultdict(lambda: {'file_touches': 0, 'commits': set()}))
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            for commit in commits:
                ...
                for file_path in files:
                    ...
                    contributor_data[author_login][task_type]['file_touches'] += 1
    ...
    for repo_name in repo_names:
        ...
        for contributor_id, task_dict in contributor_data.items():
            for task_type, file_data in task_dict.items():
                if file_data['file_touches'] > 0:
                    rows.append({
                        'repo': repo_name,
                        'contributor_id': contributor_id,
                        'task_type': task_type,
                        #'file_count': len(file_data['files']),
                        'file_count': file_data['file_touches'],
...
def main():
    ...
    parser.add_argument('--output', default='contributors_tasks_GNOME_based.csv', help='Output CSV filename')
